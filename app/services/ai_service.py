...
class RestaurantContext(BaseModel):
    ...
    current_time: str
    context_hint: Optional[str] = None  # Optionaler Hinweis vom Benutzer
...
"""
    ...
    async def suggest_tables(
        ...
        try:
            ...
            logger.info(f"AI suggested {len(suggestions)} tables, top confidence: "
    ...
    def build_context_from_data(
        ...
        for r in reservations:
            if r.get("status") in ("confirmed", "seated"):
                reservation_contexts.append(ReservationContext(
                    id=r["id"],
                    table_id=r.get("table_id"),
        ...
        for o in orders:
            if o.get("status") not in ("paid", "canceled"):
                order_contexts.append(OrderContext(
                    id=o["id"],
    ...
    async def suggest_table_for_reservation(
        ...
        suitable_tables = [
            t for t in tables
            if t.get("is_active", True) and t.get("capacity", 0) >= party_size
        ]
        ...
        table_display = table_num if str(table_num).lower().startswith("tisch") else f"Tisch {table_num}"
