...
class SumUpService:
    ...
    async def get_reader(self, merchant_code: str, reader_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"SumUp API Error getting reader: {e.response.status_code} - {e.response.text}"
            )
            raise
        except Exception as e:
    ...
    async def create_reader(
        ...
        try:
            ...
            logger.error(
                f"SumUp API Error creating reader: {e.response.status_code} - {e.response.text}"
            )
            raise
        except Exception as e:
    ...
    async def get_reader_status(self, merchant_code: str, reader_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"SumUp API Error getting reader status: {e.response.status_code} - {e.response.text}"
            )
            raise
        except Exception as e:
    ...
    async def create_checkout(
        ...
        try:
            ...
            if return_url:
                ...
                logger.warning(
                    "SumUp Checkout wird ohne return_url erstellt - Webhooks werden nicht empfangen!"
                )
            ...
            logger.error(
                f"SumUp API Error creating checkout: {e.response.status_code} - {e.response.text}"
            )
            raise
        except Exception as e:
    ...
    async def process_checkout(
        ...
        try:
            ...
            response = await self.client.put(f"/v0.1/checkouts/{checkout_id}", json=payload)
            response.raise_for_status()
            return response.json()
        except httpx.HTTPStatusError as e:
            logger.error(
                f"SumUp API Error processing checkout: {e.response.status_code} - {e.response.text}"
            )
            raise
        except Exception as e:
    ...
    async def create_merchant_checkout(
        ...
        try:
            ...
            response = await self.client.post(
                f"/v0.1/merchants/{merchant_code}/checkout", json=payload
            )
            response.raise_for_status()
            return response.json()
        except httpx.HTTPStatusError as e:
            logger.error(
                f"SumUp API Error creating merchant checkout: {e.response.status_code} - {e.response.text}"
            )
            raise
        except Exception as e:
    ...
    async def create_reader_checkout(
        ...
        try:
            ...
            response = await self.client.post(
                f"/v0.1/merchants/{merchant_code}/readers/{reader_id}/checkout", json=payload
            )
            response.raise_for_status()
            return response.json()
        except httpx.HTTPStatusError as e:
            logger.error(
                f"SumUp API Error creating reader checkout: {e.response.status_code} - {e.response.text}"
            )
            raise
        except Exception as e:
    ...
    async def terminate_reader_checkout(self, merchant_code: str, reader_id: str) -> None:
        ...
        try:
            ...
            logger.error(
                f"SumUp API Error terminating reader checkout: {e.response.status_code} - {e.response.text}"
            )
            raise
        except Exception as e:
    ...
    async def get_transaction(
        ...
        try:
            ...
            logger.error(
                f"SumUp API Error getting transaction: {e.response.status_code} - {e.response.text}"
            )
            raise
        except Exception as e:
    ...
    async def get_receipt(
        ...
        try:
            ...
            response = await self.client.get(f"/v1.1/receipts/{receipt_id}", params=params)
            response.raise_for_status()
            return response.json()
        except httpx.HTTPStatusError as e:
            logger.error(
                f"SumUp API Error getting receipt: {e.response.status_code} - {e.response.text}"
            )
            raise
        except Exception as e:
