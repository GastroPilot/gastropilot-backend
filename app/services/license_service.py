...
class LicenseService:
    ...
    def __init__(self):
        self._features: dict[str, bool] = (
            DEVELOPMENT_FEATURES.copy()
        )  # Default: alle aktiviert (für Development)
        self._package: str | None = (
            None  # Paket-Name (starter, basic, professional, business, premium)
        )
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt > 0:
                        ...
                        logger.info(
                            f"Retrying license check (attempt {attempt + 1}/{max_retries}) after {wait_time}s..."
                        )
                    ...
                    async with AsyncClient(timeout=Timeout(LICENSE_CHECK_TIMEOUT)) as client:
                        ...
                        if response.status_code == 200:
                            ...
                            if not data.get("valid", False):
                                ...
                                logger.warning(
                                    f"License check returned invalid license: {error_message}"
                                )
                    ...
                    if isinstance(e, (TimeoutError, ConnectionError)) and attempt < max_retries - 1:
                        logger.warning(
                            f"Connection error during license check ({error_type}), will retry: {e}"
                        )
                        continue
                    else:
                        logger.error(
                            f"Error during license check ({error_type}): {e}", exc_info=True
                        )
