...
from app.settings import RESERVATION_WIDGET_URL, SUMUP_API_KEY, SUMUP_MERCHANT_CODE, SUMUP_TEST_MODE

logger = logging.getLogger(__name__)

router = APIRouter(
    prefix="/restaurants/{restaurant_id}/reservations/{reservation_id}/prepayment",
    tags=["prepayments"],
)
...
async def _get_reservation_or_404(
    ...
    result = await session.execute(
        select(Reservation).where(
            Reservation.id == reservation_id, Reservation.restaurant_id == restaurant_id
        )
    )
...
async def get_checkout_url(
    ...
    if not prepayment or not prepayment.payment_id:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Keine aktive Vorauszahlung gefunden"
        )
