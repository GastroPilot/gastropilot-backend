...
from app.database.models import Order, SumUpPayment, Restaurant, ReservationPrepayment, Reservation
from app.settings import SUMUP_WEBHOOK_SECRET, SUMUP_MERCHANT_CODE, SUMUP_API_KEY
...
async def _handle_prepayment_webhook(
    ...
    logger.info(f"Processing prepayment webhook for prepayment_id: {prepayment.id}, event: {event_type}")
    ...
    if event_type_normalized in ["payment.succeeded", "payment_succeeded"]:
        ...
        logger.info(f"Prepayment erfolgreich: Reservation {prepayment.reservation_id}, Transaction {transaction_code}")
    ...
    elif event_type_normalized in ["checkout.status.updated", "checkout_status_changed", "checkout_status_updated"]:
        ...
        if checkout_status == "PAID":
            ...
            logger.info(f"Prepayment erfolgreich (Checkout): Reservation {prepayment.reservation_id}")
            ...
            logger.warning(f"Prepayment fehlgeschlagen (Checkout): Reservation {prepayment.reservation_id}")
    ...
    return {"status": "ok", "prepayment_id": prepayment.id, "reservation_id": prepayment.reservation_id}
...
async def handle_sumup_webhook(
    ...
    try:
        ...
        if "checkout" in event_type_normalized:
            logger.warning(f"=== CHECKOUT Event Debug ===")
            ...
            logger.warning(f"Data object content: {json.dumps(data, indent=2, default=str) if data else 'None'}")
            logger.warning(f"Full webhook structure: {json.dumps(webhook_data, indent=2, default=str)}")
            logger.warning(f"===========================")
        ...
        if not sumup_payment and checkout_id:
            ...
            result = await session.execute(
                select(SumUpPayment).where(
                    SumUpPayment.checkout_id == checkout_id
                )
            )
            sumup_payment = result.scalar_one_or_none()
            if sumup_payment:
                logger.info(f"Payment gefunden über checkout_id: Payment ID {sumup_payment.id}, Order ID {sumup_payment.order_id}")
        ...
        if not sumup_payment and checkout_reference:
            ...
            if checkout_reference.startswith("order_"):
                try:
                    ...
                    result = await session.execute(
                        select(SumUpPayment).where(
                            SumUpPayment.order_id == order_id
                        ).order_by(SumUpPayment.created_at_utc.desc())
                    )
                    sumup_payment = result.scalar_one_or_none()
                    if sumup_payment:
                        logger.info(f"Payment gefunden über checkout_reference: Payment ID {sumup_payment.id}, Order ID {order_id}")
                except (ValueError, IndexError) as e:
                    logger.warning(f"Fehler beim Extrahieren der Order-ID aus checkout_reference '{checkout_reference}': {e}")
                    pass
        ...
        if not sumup_payment and "checkout" in event_type_normalized:
            logger.warning(f"Fallback: Suche Payment in allen Payments der letzten 5 Minuten")
            ...
            if checkout_id:
                for payment in all_recent_payments:
                    if payment.checkout_id == checkout_id:
                        ...
                        logger.info(f"Payment gefunden über Fallback-Suche (checkout_id): Payment ID {payment.id}")
                        break
                ...
                for payment in all_recent_payments:
                    if payment.webhook_data and isinstance(payment.webhook_data, dict):
                        ...
                        if stored_reference and stored_reference == checkout_reference:
                            sumup_payment = payment
                            logger.info(f"Payment gefunden über Fallback-Suche (checkout_reference): Payment ID {payment.id}")
                            break
            ...
            if not sumup_payment:
                for payment in all_recent_payments:
                    if payment.status in ["processing", "pending"]:
                        sumup_payment = payment
                        logger.info(f"Payment gefunden über Fallback-Suche (neuestes processing Payment): Payment ID {payment.id}, Order ID {payment.order_id}, Status: {payment.status}")
                        break
        ...
        if not sumup_payment and not prepayment:
            ...
            logger.warning(f"Event Type: {event_type}")
            logger.warning(f"Data object: {json.dumps(data, indent=2, default=str) if data else 'None'}")
            logger.warning(f"Vollständiger Webhook-Payload: {json.dumps(webhook_data, indent=2, default=str)}")
            return {"status": "ignored", "reason": "payment_not_found"}
        
        # Wenn Prepayment, verarbeite es separat
        if prepayment:
            return await _handle_prepayment_webhook(
                prepayment, event_type, event_type_normalized, transaction_status,
                checkout_id, transaction_code, transaction_id, webhook_data, session
            )
        ...
        if event_type_normalized in ["payment.succeeded", "payment_succeeded"]:
            ...
            sumup_payment.completed_at = datetime.now(timezone.utc)
            ...
            logger.warning(f"Zahlung fehlgeschlagen: Order {order.id}, Transaction {transaction_code}")
            ...
            if checkout_status == "PAID":
                ...
                if (transaction_code or transaction_id) and SUMUP_MERCHANT_CODE and SUMUP_API_KEY:
                    try:
                        async with SumUpService(SUMUP_API_KEY) as sumup:
                            ...
                            if receipt_items:
                                logger.info(f"Receipt-Items gefunden: {len(receipt_items)} Produkte für Order {order.id}")
                            
                            logger.info(f"Vollständiger Receipt abgerufen für Order {order.id}: Transaction Code {transaction_code}, Receipt No: {transaction_data.get('receipt_no')}")
                            ...
                            if transaction_data.get("receipt_no"):
                                ...
                                if not sumup_payment.webhook_data:
                                    sumup_payment.webhook_data = {}
                                sumup_payment.webhook_data["receipt_no"] = transaction_data.get("receipt_no")
                                
                    except Exception as e:
                        logger.warning(f"Fehler beim Abrufen des Receipts für Order {order.id}: {e}")
                ...
                logger.info(f"Checkout erfolgreich bezahlt: Order {order.id}, Checkout {checkout_id}")
