...
from app.database.models import Order, OrderItem, Restaurant, SumUpPayment
...
class ReaderCreate(BaseModel):
    ...
    pairing_code: str = Field(
        ..., min_length=8, max_length=9, description="8-9 stelliger Pairing-Code vom Terminal"
    )
...
class ReaderStatusRead(BaseModel):
    ...
    state: str | None = None  # IDLE, WAITING_FOR_CARD, etc.
...
class PaymentRequest(BaseModel):
    ...
    reader_id: str | None = Field(None, description="Reader ID (erforderlich im Produktionsmodus)")
    amount: float = Field(..., gt=0, description="Betrag in EUR")
    currency: str = Field(default="EUR", max_length=3)
    description: str | None = None
    tip_rates: list[float] | None = Field(
        None, description="Liste von Trinkgeld-Sätzen (z.B. [0.05, 0.10, 0.15])"
    )
    tip_timeout: int | None = Field(
        None, ge=30, le=120, description="Timeout für Trinkgeld-Auswahl in Sekunden"
    )
...
class PaymentResponse(BaseModel):
    ...
    payment_id: int
    checkout_id: str | None = None  # Checkout ID für weitere Verarbeitung
    client_transaction_id: str
    reader_id: str | None = None  # Optional, da Zahlung ohne Reader-ID möglich ist
    amount: float
    currency: str
    status: str
    message: str
...
async def list_readers(
    ...
    try:
        ...
        logger.error(
            f"SumUp API Error listing readers: {e.response.status_code} - {e.response.text}"
        )
        raise HTTPException(
...
@router.post(
    "/orders/{order_id}/pay", response_model=PaymentResponse, status_code=status.HTTP_201_CREATED
)
async def start_payment(
    ...
    if order.payment_status == "paid":
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Bestellung ist bereits bezahlt"
        )
    ...
    if amount <= 0:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Betrag muss größer als 0 sein"
        )
        ...
        sumup_items.append(
            {
                ...
                "price": round(
                    net_unit_price, 2
                ),  # Netto-Preis ohne MwSt (wie SumUp Receipt erwartet)
    ...
    if not return_url:
        logger.warning(
            "SUMUP_WEBHOOK_URL nicht gesetzt - Webhooks werden nicht empfangen. "
            "Bitte setzen Sie SUMUP_WEBHOOK_URL in der .env Datei (z.B. https://api.example.com/v1/webhooks/sumup)"
        )
    else:
    ...
    try:
        async with _get_sumup_service(restaurant) as sumup:
            ...
            if order_items:
                items_summary = ", ".join(
                    [f"{item.item_name} x{item.quantity}" for item in order_items[:5]]
                )
            ...
            if SUMUP_TEST_MODE:
                ...
                if not checkout_id:
                    logger.error(
                        f"SumUp Checkout Response hat keine 'id' oder 'checkout_id': {checkout_response}"
                    )
                    raise HTTPException(
                        status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                ...
                client_transaction_id = (
                    checkout_response.get("client_transaction_id") or checkout_id
                )
                ...
                if not client_transaction_id:
                    logger.error(
                        f"SumUp Reader Checkout Response hat keine 'client_transaction_id': {checkout_response}"
                    )
                    raise HTTPException(
                        status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            ...
            logger.info(
                f"Checkout erstellt - ID: {checkout_id}, Reference: {checkout_reference}, Modus: {'TEST' if SUMUP_TEST_MODE else 'PRODUKTION'}"
            )
...
async def get_payments(
    restaurant_id: int,
    status: str | None = None,  # Filter nach Status (failed, canceled, successful, etc.)
    limit: int = 100,
    session: AsyncSession = Depends(get_session),
    current_user: User = Depends(require_mitarbeiter_role),
):
    ...
    # Query bauen
    query = select(SumUpPayment).where(SumUpPayment.restaurant_id == restaurant_id)
