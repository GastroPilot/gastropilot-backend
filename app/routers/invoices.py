...
from app.dependencies import get_session, get_current_user, require_mitarbeiter_role, require_orders_module, User
from app.database.models import Order, OrderItem, Restaurant, Table as TableModel
...
async def generate_invoice_pdf(
    ...
    try:
        ...
        # Separator Line
        story.append(HRFlowable(width="100%", thickness=1, lineCap='round', color=colors.HexColor('#000000')))
        story.append(Spacer(1, 8*mm))
        ...
        order_info_data.append([
            Paragraph(f"<b>Bestellnummer:</b> {order.order_number or f'#{order.id}'}", order_info_style),
            Paragraph(f"<b>Datum:</b> {order.opened_at.strftime('%d.%m.%Y')}", order_info_style),
        ])
        order_info_data.append([
            Paragraph(f"<b>Uhrzeit:</b> {order.opened_at.strftime('%H:%M')} Uhr", order_info_style),
            Paragraph(f"<b>Rechnungsdatum:</b> {order.paid_at.strftime('%d.%m.%Y') if order.paid_at else order.opened_at.strftime('%d.%m.%Y')}", order_info_style),
        ])
        if table_number:
            # Tischname formatieren (vermeide "Tisch Tisch 2")
            table_display = table_number if str(table_number).lower().startswith("tisch") else f"Tisch {table_number}"
            order_info_data.append([
                Paragraph(f"<b>{table_display}</b>", order_info_style),
                Paragraph(f"<b>Personen:</b> {order.party_size}" if order.party_size else "", order_info_style),
            ])
        ...
        for item in items:
            ...
            if item.notes:
                data.append([
                    Paragraph(f"<i>└ {item.notes}</i>", ParagraphStyle(
                        'ItemNote',
                        parent=normal_style,
                        fontSize=9,
                        textColor=colors.HexColor('#666666'),
        ...
        data.append(['', '', Paragraph('Zwischensumme:', normal_style), Paragraph(f"{order.subtotal:.2f} €", normal_style)])
        if order.discount_amount > 0:
            discount_label = "Rabatt"
            if order.discount_percentage:
                discount_label += f" ({order.discount_percentage}%)"
            data.append(['', '', Paragraph(discount_label + ':', normal_style), Paragraph(f"-{order.discount_amount:.2f} €", normal_style)])
        # MwSt-Aufschlüsselung nach Steuersätzen
        if order.tax_amount_7 and order.tax_amount_7 > 0:
            data.append(['', '', Paragraph('MwSt. (7%):', normal_style), Paragraph(f"{order.tax_amount_7:.2f} €", normal_style)])
        if order.tax_amount_19 and order.tax_amount_19 > 0:
            data.append(['', '', Paragraph('MwSt. (19%):', normal_style), Paragraph(f"{order.tax_amount_19:.2f} €", normal_style)])
        if order.tax_amount > 0 and (not order.tax_amount_7 or order.tax_amount_7 == 0) and (not order.tax_amount_19 or order.tax_amount_19 == 0):
            # Fallback für alte Bestellungen ohne Aufschlüsselung
            data.append(['', '', Paragraph('MwSt.:', normal_style), Paragraph(f"{order.tax_amount:.2f} €", normal_style)])
        if order.tip_amount and order.tip_amount > 0:
            data.append(['', '', Paragraph('Trinkgeld:', normal_style), Paragraph(f"{order.tip_amount:.2f} €", normal_style)])
        ...
        data.append(['', '', Paragraph('<b>GESAMT:</b>', total_style), Paragraph(f"<b>{order.total:.2f} €</b>", total_style)])
        ...
        items_table.setStyle(TableStyle([
            ...
            ('LINEBELOW', (0, summary_start_row-1), (-1, summary_start_row-1), 1, colors.HexColor('#cccccc')),
            # Total row
        ...
        if order.payment_status == "paid":
            ...
            if order.paid_at:
                payment_info.append(Paragraph(f"Bezahlt am: {order.paid_at.strftime('%d.%m.%Y um %H:%M')} Uhr", payment_info_style))
            payment_methods = {
                "cash": "Bar",
                "card": "Karte",
            ...
            if order.payment_method:
                payment_info.append(Paragraph(f"Zahlungsmethode: {payment_methods.get(order.payment_method, order.payment_method)}", payment_info_style))
            ...
            if order.payment_method == "sumup_card":
                ...
                if sumup_payment:
                    if sumup_payment.transaction_code:
                        payment_info.append(Paragraph(f"SumUp Transaction: {sumup_payment.transaction_code}", payment_info_style))
                    if sumup_payment.checkout_id:
                        payment_info.append(Paragraph(f"SumUp Checkout ID: {sumup_payment.checkout_id[:20]}...", payment_info_style))
                ...
                if isinstance(split_payments_list, list):
                    for payment in split_payments_list:
                        if isinstance(payment, dict):
                            method = payment_methods.get(payment.get('method', ''), payment.get('method', ''))
                            amount = payment.get('amount', 0)
                            payment_info.append(Paragraph(f"  • {method}: {amount:.2f} €", payment_info_style))
        ...
        if order.payment_method == "sumup_card":
            ...
            if sumup_payment:
                ...
                if sumup_payment.transaction_code:
                    story.append(Paragraph(f"Transaction Code: {sumup_payment.transaction_code}", sumup_info_style))
                if sumup_payment.checkout_id:
                    story.append(Paragraph(f"Checkout ID: {sumup_payment.checkout_id}", sumup_info_style))
                ...
                if receipt_data:
                    ...
                    story.append(Spacer(1, 3*mm))
                    story.append(HRFlowable(width="100%", thickness=0.3, lineCap='round', color=colors.HexColor('#cccccc')))
                    story.append(Spacer(1, 3*mm))
                    
                    # Receipt Number
                    if transaction_data.get("receipt_no"):
                        story.append(Paragraph(f"<b>Belegnummer:</b> {transaction_data.get('receipt_no')}", sumup_info_style))
                    
                    # Transaktionsdetails
                    if transaction_data.get("timestamp"):
                        from datetime import datetime
                        try:
                            ts = datetime.fromisoformat(transaction_data.get("timestamp").replace("Z", "+00:00"))
                            story.append(Paragraph(f"<b>Transaktionszeit:</b> {ts.strftime('%d.%m.%Y %H:%M:%S')} Uhr", sumup_info_style))
                        except:
                            story.append(Paragraph(f"<b>Transaktionszeit:</b> {transaction_data.get('timestamp')}", sumup_info_style))
                    ...
                    if transaction_data.get("payment_type"):
                        ...
                        pt = payment_types.get(transaction_data.get("payment_type"), transaction_data.get("payment_type"))
                    ...
                    if transaction_data.get("entry_mode"):
                        ...
                        em = entry_modes.get(transaction_data.get("entry_mode"), transaction_data.get("entry_mode"))
                    ...
                    if transaction_data.get("verification_method"):
                        ...
                        vm = verification_methods.get(transaction_data.get("verification_method"), transaction_data.get("verification_method"))
                    ...
                    if card_data:
                        if card_data.get("last_4_digits"):
                            story.append(Paragraph(f"<b>Karte:</b> **** {card_data.get('last_4_digits')}", sumup_info_style))
                        if card_data.get("type"):
                            story.append(Paragraph(f"<b>Kartentyp:</b> {card_data.get('type')}", sumup_info_style))
                    ...
                    if acquirer_data:
                        ...
                        if acquirer_data.get("tid"):
                            story.append(Paragraph(f"<b>Terminal ID:</b> {acquirer_data.get('tid')}", sumup_info_style))
                        if acquirer_data.get("authorization_code"):
                            story.append(Paragraph(f"<b>Autorisierungscode:</b> {acquirer_data.get('authorization_code')}", sumup_info_style))
                        if acquirer_data.get("return_code"):
                            story.append(Paragraph(f"<b>Return Code:</b> {acquirer_data.get('return_code')}", sumup_info_style))
                        if acquirer_data.get("local_time"):
                            story.append(Paragraph(f"<b>Lokale Zeit:</b> {acquirer_data.get('local_time')}", sumup_info_style))
                    ...
                    if vat_rates:
                        ...
                        story.append(HRFlowable(width="100%", thickness=0.3, lineCap='round', color=colors.HexColor('#cccccc')))
                        story.append(Spacer(1, 3*mm))
                        story.append(Paragraph("<b>MwSt-Aufschlüsselung (SumUp)</b>", sumup_header_style))
        ...
        story.append(Paragraph("Diese Rechnung ist steuerrechtlich aufbewahrungspflichtig.", footer_style))
        if restaurant.address:
            story.append(Spacer(1, 2*mm))
            story.append(Paragraph(f"Umsatzsteuer-ID gemäß §19 UStG", footer_style))
